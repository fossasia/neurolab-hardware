EESchema Schematic File Version 2
LIBS:power
LIBS:device
LIBS:switches
LIBS:relays
LIBS:motors
LIBS:transistors
LIBS:conn
LIBS:linear
LIBS:regul
LIBS:74xx
LIBS:cmos4000
LIBS:adc-dac
LIBS:memory
LIBS:xilinx
LIBS:microcontrollers
LIBS:dsp
LIBS:microchip
LIBS:analog_switches
LIBS:motorola
LIBS:texas
LIBS:intel
LIBS:audio
LIBS:interface
LIBS:digital-audio
LIBS:philips
LIBS:display
LIBS:cypress
LIBS:siliconi
LIBS:opto
LIBS:atmel
LIBS:contrib
LIBS:valves
LIBS:neuro
LIBS:neurolab-cache
EELAYER 25 0
EELAYER END
$Descr A4 11693 8268
encoding utf-8
Sheet 14 14
Title ""
Date ""
Rev ""
Comp ""
Comment1 ""
Comment2 ""
Comment3 ""
Comment4 ""
$EndDescr
Text HLabel 3440 2445 0    39   Input ~ 0
CS
Text HLabel 3440 4065 0    39   Input ~ 0
SCL
Text HLabel 6520 2455 0    39   Input ~ 0
MISO
Text HLabel 6495 4060 0    39   Input ~ 0
MOSI
Text HLabel 4960 2445 2    39   Output ~ 0
CS'
Text HLabel 4960 4065 2    39   Output ~ 0
SCL'
Text HLabel 7950 2455 2    39   Output ~ 0
MISO'
Text HLabel 7950 4060 2    39   Output ~ 0
MOSI'
$Comp
L R R47
U 1 1 5BF6BEFB
P 4160 2445
F 0 "R47" V 4240 2445 50  0000 C CNN
F 1 "1k" V 4160 2445 50  0000 C CNN
F 2 "Resistors_SMD:R_0603_HandSoldering" V 4090 2445 50  0001 C CNN
F 3 "" H 4160 2445 50  0001 C CNN
	1    4160 2445
	0    1    1    0   
$EndComp
$Comp
L R R48
U 1 1 5BF6BF8D
P 3730 2720
F 0 "R48" V 3810 2720 50  0000 C CNN
F 1 "2k" V 3730 2720 50  0000 C CNN
F 2 "Resistors_SMD:R_0603_HandSoldering" V 3660 2720 50  0001 C CNN
F 3 "" H 3730 2720 50  0001 C CNN
	1    3730 2720
	1    0    0    -1  
$EndComp
$Comp
L R R46
U 1 1 5BF6BFF5
P 4160 4065
F 0 "R46" V 4240 4065 50  0000 C CNN
F 1 "1k" V 4160 4065 50  0000 C CNN
F 2 "Resistors_SMD:R_0603_HandSoldering" V 4090 4065 50  0001 C CNN
F 3 "" H 4160 4065 50  0001 C CNN
	1    4160 4065
	0    1    1    0   
$EndComp
$Comp
L R R49
U 1 1 5BF6C0C0
P 7265 2455
F 0 "R49" V 7345 2455 50  0000 C CNN
F 1 "1k" V 7265 2455 50  0000 C CNN
F 2 "Resistors_SMD:R_0603_HandSoldering" V 7195 2455 50  0001 C CNN
F 3 "" H 7265 2455 50  0001 C CNN
	1    7265 2455
	0    1    1    0   
$EndComp
$Comp
L R R45
U 1 1 5BF6C106
P 3735 4450
F 0 "R45" V 3815 4450 50  0000 C CNN
F 1 "2k" V 3735 4450 50  0000 C CNN
F 2 "Resistors_SMD:R_0603_HandSoldering" V 3665 4450 50  0001 C CNN
F 3 "" H 3735 4450 50  0001 C CNN
	1    3735 4450
	1    0    0    -1  
$EndComp
$Comp
L R R50
U 1 1 5BF6C19D
P 6845 2775
F 0 "R50" V 6925 2775 50  0000 C CNN
F 1 "2k" V 6845 2775 50  0000 C CNN
F 2 "Resistors_SMD:R_0603_HandSoldering" V 6775 2775 50  0001 C CNN
F 3 "" H 6845 2775 50  0001 C CNN
	1    6845 2775
	1    0    0    -1  
$EndComp
$Comp
L GS2 J25
U 1 1 5BF6C4EB
P 4160 2035
F 0 "J25" H 4260 2185 50  0000 C CNN
F 1 "GS2" H 4260 1886 50  0000 C CNN
F 2 "Connectors:GS2" V 4234 2035 50  0001 C CNN
F 3 "" H 4160 2035 50  0001 C CNN
	1    4160 2035
	0    1    1    0   
$EndComp
$Comp
L GS2 J27
U 1 1 5BF6C52C
P 4150 3625
F 0 "J27" H 4250 3775 50  0000 C CNN
F 1 "GS2" H 4250 3476 50  0000 C CNN
F 2 "Connectors:GS2" V 4224 3625 50  0001 C CNN
F 3 "" H 4150 3625 50  0001 C CNN
	1    4150 3625
	0    1    1    0   
$EndComp
$Comp
L GS2 J28
U 1 1 5BF6C593
P 7265 2075
F 0 "J28" H 7365 2225 50  0000 C CNN
F 1 "GS2" H 7365 1926 50  0000 C CNN
F 2 "Connectors:GS2" V 7339 2075 50  0001 C CNN
F 3 "" H 7265 2075 50  0001 C CNN
	1    7265 2075
	0    1    1    0   
$EndComp
$Comp
L Conn_02x03_Odd_Even J26
U 1 1 5BF6C9E8
P 6435 5680
F 0 "J26" H 6485 5880 50  0000 C CNN
F 1 "ICSP Port" H 6485 5480 50  0000 C CNN
F 2 "Pin_Headers:Pin_Header_Straight_2x03_Pitch2.54mm" H 6435 5680 50  0001 C CNN
F 3 "" H 6435 5680 50  0001 C CNN
	1    6435 5680
	1    0    0    -1  
$EndComp
Text HLabel 3440 4840 0    39   Input ~ 0
GND
Text HLabel 5900 5780 0    39   Input ~ 0
RST
Wire Wire Line
	3440 2445 4010 2445
Wire Wire Line
	3730 2035 3730 2570
Connection ~ 3730 2445
Wire Wire Line
	3960 2035 3730 2035
Wire Wire Line
	4360 2035 4580 2035
Wire Wire Line
	4580 2035 4580 2445
Text HLabel 3440 3040 0    39   Input ~ 0
GND
Wire Wire Line
	3440 3040 3730 3040
Wire Wire Line
	3730 3040 3730 2870
Wire Wire Line
	4310 2445 4960 2445
Connection ~ 4580 2445
Text HLabel 6495 3055 0    39   Input ~ 0
GND
Wire Wire Line
	3440 4065 4010 4065
Wire Wire Line
	3735 3625 3735 4300
Connection ~ 3735 4065
Wire Wire Line
	3735 3625 3950 3625
Wire Wire Line
	4350 3625 4590 3625
Wire Wire Line
	4590 3625 4590 4065
Wire Wire Line
	4310 4065 4960 4065
Wire Wire Line
	3440 4840 3735 4840
Wire Wire Line
	3735 4840 3735 4600
Connection ~ 4590 4065
Wire Wire Line
	7065 2075 6845 2075
Wire Wire Line
	6845 2075 6845 2625
Wire Wire Line
	6520 2455 7115 2455
Connection ~ 6845 2455
Wire Wire Line
	7465 2075 7645 2075
Wire Wire Line
	7645 2075 7645 2455
Wire Wire Line
	7415 2455 7950 2455
Wire Wire Line
	6495 3055 6845 3055
Wire Wire Line
	6845 3055 6845 2925
Connection ~ 7645 2455
Text HLabel 5900 5680 0    39   Output ~ 0
SCL'
Text HLabel 7125 5780 2    39   Input ~ 0
GND
Text HLabel 7125 5680 2    39   Output ~ 0
MISO'
Wire Wire Line
	7125 5680 6735 5680
Text Notes 6750 5680 0    30   Italic 0
MOSI
Wire Wire Line
	5900 5780 6235 5780
Wire Wire Line
	6235 5680 5900 5680
Text Notes 6025 5680 0    30   Italic 0
SCL/SCK
Text Notes 6025 5780 0    30   Italic 0
RESET
Wire Wire Line
	7125 5780 6735 5780
Text Notes 6750 5780 0    30   Italic 0
GND
Text HLabel 7125 5580 2    39   Input ~ 0
VCC
Wire Wire Line
	7125 5580 6735 5580
Text Notes 6750 5580 0    30   Italic 0
VCC
Text HLabel 5900 5580 0    39   Output ~ 0
MOSI'
Wire Wire Line
	5900 5580 6235 5580
Text Notes 6025 5580 0    30   Italic 0
MISO
$Comp
L Conn_01x02 J29
U 1 1 5BF59BF1
P 4865 5660
F 0 "J29" H 4865 5760 50  0000 C CNN
F 1 "Teensy Port" H 4865 5460 50  0000 C CNN
F 2 "" H 4865 5660 50  0001 C CNN
F 3 "" H 4865 5660 50  0001 C CNN
	1    4865 5660
	1    0    0    -1  
$EndComp
Text HLabel 3775 5660 0    39   Output ~ 0
MOSI'
Wire Wire Line
	3775 5660 4665 5660
Text Notes 4010 5660 0    30   Italic 0
MISO (12) Teensy 3.2
Text HLabel 3775 5760 0    39   Output ~ 0
SCL'
Wire Wire Line
	3775 5760 4665 5760
Text Notes 4010 5760 0    30   Italic 0
SCL (13) Teensy 3.2
Wire Wire Line
	6495 4060 7950 4060
$EndSCHEMATC
